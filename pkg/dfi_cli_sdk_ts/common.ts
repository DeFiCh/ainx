...
export class TokenAmount extends ValueType<string> {
  ...
  constructor(tokenAmountStr: string) {
    ...
    if (res.length != 2) {
      TokenAmount._throwInvalidFormatError(tokenAmountStr);
    }
  ...
  private static _validate(token: string, amount: number) {
    if (token.length < 1 || token.length > 16) {
      this._throwInvalidFormatError(token);
    }
    if (!Number.isFinite(amount) && amount < 0) {
      this._throwInvalidFormatError(amount);
    }
  }

  private static _throwInvalidFormatError(data: any) {
    throw new Error(`invalid token value format: ${data}`);
  }
