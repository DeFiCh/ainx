...
import { BlockHeight } from "@pkg/dfi_sdk_ts/common.ts";``
function loadOpts() {
  const alias = {
    "help": "h",
    "addr": "a",
  ...
  const getHelpText = () => {
    ...
    text += "      --minconf           Min confirmations of utxo req. to be considered\n";
    text += "      --maxconf           Max confirmations of utxo req. to be considered\n";
    text += "      --minamount         Min amount value of utxo req. to be considered\n";
    text += "      --maxamount         Min amount value of utxo req. to be considered\n";
    text += "      --maxcount          Max number of utxos in one consolidation TX\n";
    return text;
  };
...
async function main() {
  ...
  while (true) {
      ...
      { maximumAmount: maxAmount, maximumCount: maxCount, minimumAmount: minAmount });
      ...
      const inTxs = txs.map(x => { return { txid: x.txid, vout: x.vout }});
      ...
      // fee approximation
      const fee = minRelayFee * ((dummyTx.length / 1000) + 1) + (dummyTx.length * feeRate);
