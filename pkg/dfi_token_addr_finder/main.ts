...
import { CliDriver, GetPoolPairResponse, initTimedLogger, ListTokensItem, TokenAmount } from "@pkg/dfi_sdk_ts/lib.ts";
...
async function run(cli: CliDriver, filter: string) {
  ...
  console.log("note: logaccountbalances does not include pool pair rewards\n\
  ...
  const tokenIds: { id: string; symbol: string; details: ListTokensItem | GetPoolPairResponse }[] = [];
  ...
  for (const addr in x.accounts) {
    ...
    const res = t.map(x => new TokenAmount(x))
      ...
      .map(x => { return { addr, val: TokenAmount.from(x.amount(), tokenIds.find(v => v.id == x.token())!.symbol) } });
  ...
  for (const [k, v] of resultGroups) {
    const m = v.map((x) => { return { addr: x.addr, val: x.val } });
    m.sort((a, b) => b.val.amount() - a.val.amount());
    const total = m.reduce((acc, x) => acc + x.val.amount(), 0);
    const res = { token: k, count: m.length, total, items: m.map(x => { return { addr: x.addr, val: x.val.toString() } }) };
...
function handleArgs() {
  ...
  const printHelpText = () => {
    let text = "Usage: <program> --token_filter=<token_substring>\n\n";
    text += "  -h, --help              Print this help message\n";
    text += "  -f, --filter=<string>   Token filter substring.\n";
    text += "                          This string is checked for substring match against token symbols\n";
    text += "                          Eg: --filter=DFI\n";
    console.log(text);
  }
